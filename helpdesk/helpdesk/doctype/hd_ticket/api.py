...
def get_meta(template: str):
    try:
        # Get all fields from HD Ticket doctype
        ...
        frappe.logger().info(f"DocFields found: {len(fields)}")

        # Filter out section breaks, column breaks, and tab breaks as they're not data fields
        fields = [f for f in fields if f.get("fieldtype") not in ["Section Break", "Column Break", "Tab Break"]]

        frappe.logger().info(f"Filtered fields: {len(fields)}")
        ...
        custom_fields = (
            ...
            .where(CustomField.dt == "HD Ticket")
            .run(as_dict=True)
        )

        frappe.logger().info(f"Custom fields found: {len(custom_fields)}")

        fields.extend(custom_fields)

        frappe.logger().info(f"Total fields: {len(fields)}")

        # Normalize field properties to match what frontend expects
        for field in fields:
            # Convert reqd to required
            field["required"] = field.get("reqd", False)
            if "reqd" in field:
                del field["reqd"]

            # Convert read_only to readonly
            field["readonly"] = field.get("read_only", False)
            if "read_only" in field:
                del field["read_only"]

            # Set disabled to False by default
            field["disabled"] = False

            # Set placeholder to empty string if not set
            field["placeholder"] = field.get("placeholder", "")

            # Set url_method to empty string if not set
            field["url_method"] = field.get("url_method", "")

        frappe.logger().info(f"Returning {len(fields)} fields")
        return fields
    except Exception as e:
        frappe.log_error(f"Error in get_meta: {str(e)}")
        frappe.logger().error(f"Error in get_meta: {str(e)}")
        # Return basic fallback fields
        return [
            {"fieldname": "subject", "fieldtype": "Data", "label": "Subject", "required": True, "readonly": False, "disabled": False, "placeholder": "", "url_method": "", "options": ""},
            {"fieldname": "description", "fieldtype": "Text Editor", "label": "Description", "required": False, "readonly": False, "disabled": False, "placeholder": "", "url_method": "", "options": ""},
            {"fieldname": "status", "fieldtype": "Link", "label": "Status", "options": "HD Ticket Status", "required": False, "readonly": False, "disabled": False, "placeholder": "", "url_method": ""},
            {"fieldname": "priority", "fieldtype": "Link", "label": "Priority", "options": "HD Ticket Priority", "required": False, "readonly": False, "disabled": False, "placeholder": "", "url_method": ""},
            {"fieldname": "ticket_type", "fieldtype": "Link", "label": "Ticket Type", "options": "HD Ticket Type", "required": False, "readonly": False, "disabled": False, "placeholder": "", "url_method": ""},
            {"fieldname": "agent_group", "fieldtype": "Link", "label": "Team", "options": "HD Team", "required": False, "readonly": False, "disabled": False, "placeholder": "", "url_method": ""},
            {"fieldname": "customer", "fieldtype": "Link", "label": "Customer", "options": "HD Customer", "required": False, "readonly": False, "disabled": False, "placeholder": "", "url_method": ""},
        ]
