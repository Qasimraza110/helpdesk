...
class HDTicket(Document):
    def get(self, *args, **kwargs):
        """Override the default get method to include fields metadata"""
        data = super().get(*args, **kwargs)
        # Add fields metadata like the get_one API does
        try:
            from helpdesk.helpdesk.doctype.hd_ticket.api import get_meta
            template = self.template or "Default"
            data["fields"] = get_meta(template)
        except Exception as e:
            # Fallback: include basic fields if get_meta fails
            data["fields"] = [
                {"fieldname": "subject", "fieldtype": "Data", "label": "Subject", "required": True, "readonly": False, "disabled": False, "placeholder": "", "url_method": ""},
                {"fieldname": "description", "fieldtype": "Text Editor", "label": "Description", "required": False, "readonly": False, "disabled": False, "placeholder": "", "url_method": ""},
